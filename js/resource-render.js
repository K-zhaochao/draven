document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderCategories() {
    filterContainer.innerHTML = resourcesData.categories
      .map(
        (cat, index) => `
  ...
  // 4. 使用事件代理绑定卡片点击事件（减少大量事件监听器，提升性能）
  function bindCardEvents() {
    gridContainer.addEventListener("click", function (e) {
      const card = e.target.closest(".resource-card");
      if (!card) return;
      const id = card.getAttribute("data-id");
      const data = resourcesData.resources.find((r) => r.id === id);
              ...
              <h5 class="modal-title" style="font-family: 'Ma Shan Zheng', serif; font-size: 1.5rem; color: var(--color-cyan-gray);"></h5>
                    ...
                    <div class="modal-footer border-0 justify-content-center pb-4">
                        <a href="#" id="custom-modal-link-btn" class="btn btn-primary btn-lg px-5" target="_blank">
                            <i class="fas fa-external-link-alt mr-2"></i>前往修炼
                        </a>
                    </div>
                </div>
            </div>
